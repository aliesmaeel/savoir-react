...
export default function FooterLatestListings() {
  ...
  return (
    <div className="flex flex-col items-start gap-[20px] w-full max-w-[315px]">
      <p className="text-[#C6A45A] text-[24px] font-semibold">LATEST LISTINGS</p>
        ...
        {items.map((item: any, index: number) => (
          <Link to={`/projects/1`} key={index} className="flex items-center gap-[10px] w-full">
            <img
              src={item.image}
              alt=""
              className="w-[99px] h-[84px] rounded-[12px] object-cover"
            />
            <div className="flex flex-col items-start gap-[11px] w-full">
              <p className="text-[18px]">{item.title}</p>
              <p className="text-[#C6A45A] text-[18px]">{item.prise}</p>
            </div>
          </Link>
        ))}
      </div>
