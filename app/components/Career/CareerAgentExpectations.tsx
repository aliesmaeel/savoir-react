...
export default function CareerAgentExpectations() {
  ...
  return (
    <div
      className=" w-full bg-center bg-cover px-[51px] pt-[60px] pb-[18px] relative"
    ...
    >
      <div className="w-full max-w-[1226px] mx-auto flex flex-col items-start justify-center gap-[30px]">
        <p className="text-white text-[74px]">Agent Expectations:</p>
        <div className="flex flex-col items-start gap-[30px] w-full pl-[25px]">
          {items.map((item: string, index: number) => (
            <ul className="flex items-start gap-[18px] w-full list-disc">
              <li className="text-white text-[27px]">{item}</li>
            </ul>
          ))}
        </div>
        <div className="px-[35px] py-[37px] rounded-[20px] w-full bg-[#FFFFFF40] backdrop-blur-[10px]">
          <p className="text-white text-[31px] font-semibold">
